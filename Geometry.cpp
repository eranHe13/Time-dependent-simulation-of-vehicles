...
double Point::getDistance(const Point& from, const Point& to) {
    return sqrt(pow(from.x - to.x,2) + pow(from.y - to.y,2)) * 100;
}

double Point::getAngle(const Point& from, const Point& to ) {
